...
const drawAbilityColumn = (
  ...
  ABILITIES.forEach((ability) => {
    ...
    page.drawText(String(character.finalAbilities[ability]), {
      x: x + 27,
      y: y - 5,
      size: 13,
      font: bold,
    ...
    page.drawText(mod, {
      x: x + 51 - bold.widthOfTextAtSize(mod, 8) / 2,
      y: y - 3.5,
      size: 8,
      font: bold,
      color: rust,
...
const drawHitPoints = (
  ...
  page.drawText(hp, {
    x: x + width - 7 - bold.widthOfTextAtSize(hp, 9),
    y: y + 5,
    size: 9,
    font: bold,
    color: ink,
...
const drawSkills = (
  ...
  for (const skill of SKILLS) {
    ...
    page.drawText(score, {
      x: x + width - bold.widthOfTextAtSize(score, 7),
      y,
      size: 7,
      font: bold,
      color: ink,
...
const drawItems = (
  ...
  const sections: Feature[] = [
    ...(character.magic.length
      ? [{ name: "Magic & Powers", summary: character.magic.join(" | ") }]
      : []),
...
const drawCharacter = (
  ...
  page.drawLine({
    ...
    end: { x: marginX + cardWidth - 8, y: headerY - 7 },
    thickness: 0.65,
    color: rule,
  });
  ...
  const itemsWidth =
    cardWidth - 20 - abilityWidth - skillsWidth - featuresWidth - gap * 3;
  const x1 = marginX + 10;
