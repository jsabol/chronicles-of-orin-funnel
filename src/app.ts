...
function card(c:CharacterRecordV1){const d=deriveCharacter(c);return `<article class="character-card ${d.status}" data-open="${c.id}" tabindex="0">${selecting?`<label class="check"><input type="checkbox" data-select="${c.id}" ${selected.has(c.id)?"checked":""}><span>Select</span></label>`:""}<div class="fate-copy"><h3>${esc(c.name)}</h3><p>${esc(d.ancestry.name)} <i>◆</i> ${esc(d.occupation.name)}</p><div class="fate-vitals"><span class="hp">${d.maxHp} HP</span><span class="def">${d.armorClass} DEF</span></div></div><div class="fate-seal"><img src="${asset(d.status==="living"?"living-sun.webp":"fallen-mark.webp")}" alt=""><strong>${d.status==="living"?"Living":"Fallen"}</strong></div><button class="icon-button" data-delete="${c.id}" aria-label="Delete ${esc(c.name)}"><img src="${asset("fa-trash.svg")}" alt=""></button></article>`}
function roster(){
