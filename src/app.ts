...
function shell(body:string){
 root.innerHTML=`<div class="site-shell"><header class="masthead"><a class="brand" href="#/" aria-label="Chronicles of Orrin"><img src="${asset("logo.png")}" alt="Chronicles of Orrin"></a><div class="sun-mark"></div></header>${store.warning?`<div class="warning" role="alert">${esc(store.warning)}</div>`:""}<main>${body}</main><footer>Made for the wastes · <a href="https://www.dndbeyond.com/srd" target="_blank" rel="noreferrer">SRD 5.1 · CC BY 4.0</a></footer></div>`
}
